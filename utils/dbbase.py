...
class DBModel(object):
    ...
    def create(self):
        _sql = self._sql_insert.format(table=self._table, columns=','.join(self._columns_add),
                                       values=','.join(['%s'] * len(self._columns_add)))
    ...
    def update(self):
        ...
        _sql = self._sql_update.format(table=self._table, values=','.join(_values), pk=self._pk)
